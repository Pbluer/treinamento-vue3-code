<template>
  ...
  <div class="mt-16">
    <form @submit.prevent="handleSubmit">
      ...
      class="px-8 py-3 mt-10 text-2x-1 font-bold text-white rounded-full bg-brand-main focus:outline-none transition-all duration-150">
    ...
    async function handleSubmit () {
      try {
        ...
        if (errors.status === 400) {
          toast.error('Ocorreu um erro ao fazer o logi')
        }
        state.isLoading = false
      } catch (error) {
        state.isLoading = false
